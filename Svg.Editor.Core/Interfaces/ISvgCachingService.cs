...
namespace Svg.Editor.Interfaces
{
    ...
    {
        private SizeF _imageDimension  = SizeF.Create(120, 120);
        ...
        public Func<string, SaveAsPngOptions, string> NamingConvention { get; protected set; } = (key, op) => $"{Path.GetFileNameWithoutExtension(key)}_{(int) op.ImageDimension.Width}px_{(int) op.ImageDimension.Width}px{op.CustomPostFix?.Invoke(key, op)}.png";
