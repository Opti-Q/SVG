    ...
    public class PathStatistics
    {
        ...
        private class LineSegment : ISegment
        {
            ...
            public void LocationAngleAtOffset(double offset, out PointF point, out float rotation)
            {
                ...
                point = SvgSetup.Factory.CreatePointF((float)(_start.X + (offset / _length) * (_end.X - _start.X)),
        ...
        private class CubicBezierSegment : ISegment
        {
            ...
            {
                return SvgSetup.Factory.CreatePointF((float)(Math.Pow(1 - t, 3) * p0.X + 3 * Math.Pow(1 - t, 2) * t * p1.X +
            ...
            {
                return SvgSetup.Factory.CreatePointF((float)(3 * Math.Pow(1 - t, 2) * (p1.X - p0.X) + 6 * (1 - t) * t * (p2.X - p1.X) + 3 * Math.Pow(t, 2) * (p3.X - p2.X)),
